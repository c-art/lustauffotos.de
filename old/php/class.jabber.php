...
/***************************************************************************
	...
	last modified: 20.09.2002 16:20:03
...
class Jabber
{
	...
	function Connect()
	{
		$this->connection = fsockopen($this->server, $this->port);
	...
	function SendAuth()
	{
		...
		if ($this->GetInfoFromIqType($packet) == "result" && $this->GetInfoFromIqId($packet) == $auth_id) {
			...
			if ($this->enable_logging) {
				$this->logfile[] = "<b>Error:</b> AuthSend() #1";
			}
	...
	function Listen()
	{
		...
		while ($line = fread($this->connection, 4096)) {
			$incoming .= $line;
		}
	...
	function RosterUpdate()
	{
		...
		$this->SendIq(NULL, "get", $roster_request_id, "jabber:iq:roster");
		sleep(1);

		$this->Listen();

		$incoming_array = $this->GetFromQueueById("iq", $roster_request_id);
	...
	function RosterAddUser($jid = NULL, $id = NULL, $name = NULL)
	{
		...
		if ($jid) {
			...
			$this->SendIq(NULL, "set", $id, "jabber:iq:roster", $payload);
			sleep(1);
			$this->Listen();

			$packet = $this->GetFromQueueById("iq", $id);
	...
	function RosterRemoveUser($jid = NULL, $id = NULL)
	{
		if ($jid && $id) {
			...
			$this->SendIq(NULL, "set", $id, "jabber:iq:roster", "<item jid='$jid' subscription='remove'/>");
			sleep(1);
			$this->Listen();
